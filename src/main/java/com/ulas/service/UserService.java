...
import com.ulas.dto.request.LoginRequestDto;
...
public class UserService implements IServiceCrud<User>  {
    ...
    public User createUser(String name, String surname, String password, String email) {
        User user = User.builder().name(name).surname(surname).password(password).email(email).build();
        return userRepository.save(user);
    }
    public User register(UserRegisterRequestDto dto){
        User user = User.builder().name(dto.getName()).surname(dto.getSurname())
                .email(dto.getEmail())
    ...
    public User register2(UserRegisterRequestDto dto){
        ...
        if(!dto.getPassword().equals(dto.getRePassword())){
            throw new RuntimeException("Girdiginiz sifreler uyusmamaktadir");
        }
    ...
    public LoginResponseDto login(LoginRequestDto dto) {
        Optional<User> user = userRepository.findOptionalByEmailAndAndPassword(dto.getEmail(), dto.getPassword());
        if(user.isEmpty()){
